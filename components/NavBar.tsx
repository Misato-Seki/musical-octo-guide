  ...
  return (
    <>
              ...
              >
                <span className={`inline-block transition-all duration-300 ${
                  pathname === item.path
                    ...
                    : 'text-gray-600 group-hover:text-black group-hover:translate-x-2 group-hover:font-bold'
                }`}>
                  {item.name}
                </span>
      ...
      <nav className="md:hidden fixed top-0 left-0 right-0 z-50 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            <div className="flex-shrink-0">
              <Link href="/" className="text-xl font-bold text-gray-600 hover:text-black transition-colors">
                Misato Seki
              </Link>
            </div>
            ...
            <div className="md:hidden">
              <button
                ...
                className="inline-flex items-center justify-center p-2 rounded-md text-gray-600 hover:text-black focus:outline-none transition-colors"
                aria-expanded="false"
              >
        ...
        <div className={`${isOpen ? 'block' : 'hidden'} md:hidden transition-all duration-300 ease-in-out bg-white`}>
          <div className="px-2 pt-2 pb-3 space-y-4 sm:px-3">
            {navItems.map((item) => (
              ...
              >
                ...
                <span className={`absolute left-0 top-1/2 -translate-y-1/2 h-[1px] bg-black transition-all duration-300 ${
                  pathname === item.path
                    ? 'w-6 opacity-100'
                    : 'w-2 opacity-40 group-hover:w-4 group-hover:opacity-60'
                }`}></span>
